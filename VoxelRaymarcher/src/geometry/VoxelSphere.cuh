...
{
public:
	static void generateVoxelSphere(VoxelSceneCPU& voxelScene, uint32_t xPos, uint32_t yPos, uint32_t zPos, uint32_t radius)
	{
			...
			for (uint32_t y = yMin; y < yMax; y++)
			{
				for (uint32_t z = zMin; z < zMax; z++)
				{
					...
					if (distanceSquared < radius * radius && distanceSquared >(radius - 1) * (radius - 1))
					{
						...
						uint32_t voxelColor = voxelfunc::generateRGBColor(std::min(red, 255u), std::min(green, 255u), std::min(blue, 255u));
						voxelScene.insertVoxel(x, y, z, voxelColor);
					}
				}
			}
		}
	...
	static void generateCheckeredVoxelSphere(VoxelSceneCPU& voxelScene, uint32_t xPos, uint32_t yPos, uint32_t zPos, uint32_t radius)
	{
		uint32_t xMin = xPos - radius;
		uint32_t xMax = xPos + radius;
		uint32_t yMin = yPos - radius;
