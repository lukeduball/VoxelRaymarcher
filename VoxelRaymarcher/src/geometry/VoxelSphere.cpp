...
void VoxelSphere::generateVoxelSphere(std::unordered_map<uint32_t, uint32_t>& voxelTable, uint32_t xPos, uint32_t yPos, uint32_t zPos, uint32_t radius)
{
		...
		for (uint32_t y = yMin; y < yMax; y++)
		{
			for (uint32_t z = zMin; z < zMax; z++)
			{
				...
				if (distanceSquared < radius * radius && distanceSquared > (radius - 1) * (radius - 1))
				{
					...
					voxelTable[VoxelFunctions::generate3DPoint(x, y, z)] = VoxelFunctions::generateRGBColor(red, green, blue);
				}
			}
		}
	}
